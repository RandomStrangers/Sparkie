...
namespace Sharkbite2.Irc2
{
	...
	/// </summary>
	public static class Rfc2812Util2 
	{
        // Regex that matches a legal IRC nick 
        public static readonly Regex nickRegex;
        //Regex to create a UserInfo2 from a string
		...
		/// into a UserInfo2 object.
		/// </summary>
		/// <param name="fullUserName">The user in nick!user@host form.</param>
		/// <returns>A UserInfo2 object.</returns>
		public static UserInfo2 UserInfoFromString( string fullUserName ) 
		{
			string[] parts = ParseUserInfoLine( fullUserName );
			if( parts == null ) 
			{
				return UserInfo2.Empty;
			}
			else 
			{
				return new UserInfo2( parts[0], parts[1], parts[2] );
			}
		}
		/// <summary>
		...
		public static bool IsValidChannelName(string channel) 
		{
			...
			if( Rfc2812Util2.ContainsSpace(channel) ) 
			{
				return false;
			}
		...
		public static bool IsValidNick( string nick) 
		{
			...
			if( Rfc2812Util2.ContainsSpace( nick ) ) 
			{
				return false;
			}
