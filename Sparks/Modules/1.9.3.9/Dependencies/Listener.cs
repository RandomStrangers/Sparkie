...
{
	public delegate void ReplyEventHandler( ReplyCode2 code, string message );
	public delegate void ErrorMessageEventHandler( ReplyCode2 code, string message );
	...
	public delegate void PublicNoticeEventHandler( UserInfo2 user, string channel, string notice );
	public delegate void PrivateNoticeEventHandler( UserInfo2 user, string notice );
	public delegate void JoinEventHandler( UserInfo2 user, string channel );
	public delegate void PublicActionEventHandler( UserInfo2 user, string channel, string description );
	public delegate void PrivateActionEventHandler( UserInfo2 user, string description );
	public delegate void PublicMessageEventHandler( UserInfo2 user, string channel, string message );
	public delegate void PrivateMessageEventHandler( UserInfo2 user, string message );
	public delegate void NickEventHandler( UserInfo2 user, string newNick );
	public delegate void PartEventHandler( UserInfo2 user, string channel, string reason);
	public delegate void QuitEventHandler( UserInfo2 user, string reason);
	public delegate void InviteEventHandler( UserInfo2 user, string channel );
	public delegate void KickEventHandler( UserInfo2 user, string channel, string kickee, string reason );
	public delegate void NamesEventHandler( string channel, string[] nicks, bool last );
	public delegate void ChannelModeChangeEventHandler( UserInfo2 who, string channel );
	public delegate void KillEventHandler( UserInfo2 user, string nick, string reason );
	...
	public sealed class Listener
	{
        ...
        public void Parse(string message ) 
		{
			...
			{
				tokens[1] = RemoveLeadingColon( tokens[1] );
				Error( ReplyCode2.IrcServerError, CondenseStrings(tokens, 1) );
			}
        ...
        /// </summary>
        public void Error( ReplyCode2 code, string message ) 
		{
			if( OnError != null )
			{
		...
		{	
			...
			switch( tokens[1] ) 
			{
				case NOTICE:		
					...
					if( Rfc2812Util2.IsValidChannelName( tokens[2] ) )
					{			
						if( OnPublicNotice != null )
						{
							OnPublicNotice(
								Rfc2812Util2.UserInfoFromString( tokens[0] ),
								tokens[2],
				...
				case JOIN:
					if( OnJoin != null )
					{
						OnJoin( Rfc2812Util2.UserInfoFromString( tokens[0] ), RemoveLeadingColon( tokens[2] ) );
					}
					break;
				case PRIVMSG:
					tokens[3] = RemoveLeadingColon( tokens[3] );
					if( tokens[3] == ACTION ) 
					{
						if( Rfc2812Util2.IsValidChannelName( tokens[2] ) )
						{
							if( OnAction != null ) 
							{
								...
								OnAction( Rfc2812Util2.UserInfoFromString( tokens[0] ),tokens[2],CondenseStrings( tokens, 4) );
							}
						}
						else 
						{
							if( OnPrivateAction != null ) 
							{
								int last = tokens.Length - 1;
								tokens[ last ] = RemoveTrailingQuote( tokens[last] );
								OnPrivateAction( Rfc2812Util2.UserInfoFromString( tokens[0] ),CondenseStrings( tokens, 4) );
							}
						}
					}
					else if( channelPattern.IsMatch( tokens[2] ) )
					{
						if( OnPublic != null )
						{
							OnPublic(Rfc2812Util2.UserInfoFromString( tokens[0] ),tokens[2],CondenseStrings( tokens, 3) );
						}
					}
					else 
					{
						if( OnPrivate != null )
						{
							OnPrivate(Rfc2812Util2.UserInfoFromString( tokens[0] ), CondenseStrings( tokens, 3) );
						}
					}
					break;
				case NICK:
					if( OnNick != null )
					{
						OnNick(	Rfc2812Util2.UserInfoFromString( tokens[0] ), RemoveLeadingColon( tokens[2] ) );
					}
					break;
				case PART:
					if( OnPart != null )					
				...
				case QUIT:
					if( OnQuit != null ) 
					{
						...
						OnQuit( Rfc2812Util2.UserInfoFromString( tokens[0] ), CondenseStrings( tokens, 2) );
					}
					break;
				case INVITE:
					if( OnInvite != null ) 
					{
						OnInvite(
							Rfc2812Util2.UserInfoFromString( tokens[0] ), RemoveLeadingColon( tokens[3] ) );
					}
					break;
				case KICK:
					if( OnKick != null )
					{	
						...
						OnKick(Rfc2812Util2.UserInfoFromString( tokens[0] ),tokens[2],tokens[3], CondenseStrings( tokens, 4) );
					}
					break;
				case MODE:
					if( channelPattern.IsMatch( tokens[2] ) )
					{
						...
						{
							UserInfo2 who = Rfc2812Util2.UserInfoFromString( tokens[0] );
				...
				case KILL:
					if( OnKill != null )
					{
						...
						OnKill( Rfc2812Util2.UserInfoFromString( tokens[0] ), tokens[2], reason );
					}
					break;
				default: 
					if( OnError != null ) 
					{
						OnError( ReplyCode2.UnparseableMessage, CondenseStrings( tokens, 0 ) );
					}
					break;
			}
		}
		...
		{
			ReplyCode2 code = (ReplyCode2) int.Parse( tokens[1], CultureInfo.InvariantCulture );
			tokens[3] = RemoveLeadingColon( tokens[3] );
			switch( code )
			{
				//Messages sent upon successful registration 
				case ReplyCode2.RPL_WELCOME:
				case ReplyCode2.RPL_YOURESERVICE:
					if( OnRegistered != null ) 
					{
						OnRegistered();
					}
					break;	
				case ReplyCode2.RPL_NAMREPLY:
					if ( OnNames != null ) 
					{
					...
					break;
				case ReplyCode2.RPL_ENDOFNAMES:
					if( OnNames != null ) 
					{
						OnNames( tokens[3], new string[0], true );
					}
					break;
				case ReplyCode2.ERR_NICKNAMEINUSE:
				case ReplyCode2.ERR_NICKCOLLISION:
					if ( OnNickError != null ) 
					{
						tokens[4] = RemoveLeadingColon( tokens[4] );
		...
		void HandleDefaultReply( ReplyCode2 code, string[] tokens ) 
		{
			if (code >= ReplyCode2.ERR_NOSUCHNICK && code <= ReplyCode2.ERR_USERSDONTMATCH) 
			{
				if( OnError != null )
				{
