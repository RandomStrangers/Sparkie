...
namespace Sharkbite2.Irc2
{
	...
	public sealed class Connection
	{
		...
		void MyNickChanged(UserInfo2 user, string newNick)
		{
			if ( Nick == user.Nick )
			{
				Nick = newNick;
		...
		string GetNewNick()
		{
			...
			NameGenerator2 generator = new NameGenerator2();
			string name;
			do
			{
				name = generator.MakeName();
			}
			while (!Rfc2812Util2.IsValidNick(name) || name.Length == 1);
		...
		{
			if ( !Rfc2812Util2.IsValidNick( nick ) )
