...
namespace GoldenSparks.Modules.Relay2.IRC2 
{
    ...
    public sealed class IRCBot2 : RelayBot2 
    {
        ...
        void OnAction(UserInfo2 user, string channel, string description) {
            MessageInGame(user.Nick, string.Format("&I(IRC2) * {0} {1}", user.Nick, description));
        }
        
        void OnJoin(UserInfo2 user, string channel) {
            conn.SendNames(channel);
            AnnounceJoinLeave(user.Nick, "joined", channel);
        }
        
        void OnPart(UserInfo2 user, string channel, string reason) {
        ...
        void OnQuit(UserInfo2 user, string reason) {
        ...
        void OnError(ReplyCode2 code, string message) {
            Logger.Log(LogType.RelayActivity, "IRC2 Error: " + message);
        }

        void OnPrivate(UserInfo2 user, string message) {
        ...
        void OnPublic(UserInfo2 user, string channel, string message) {
            RelayUser2 rUser = new RelayUser2();
            rUser.ID        = user.Nick;
            rUser.Nick      = user.Nick;
        ...
        void OnPrivateNotice(UserInfo2 user, string notice) {
        ...
        void OnNick(UserInfo2 user, string newNick) {
        ...
        void OnChannelModeChange(UserInfo2 who, string channel) {
            conn.SendNames(channel);
        }
        
        void OnKick(UserInfo2 user, string channel, string kickee, string reason) {
            nicks.OnLeftChannel(user, channel);
        ...
        void OnKill(UserInfo2 user, string nick, string reason) {
            nicks.OnLeft(user);
        }
