...
{
    [Obsolete("Use CmdFreeze instead.")]
    public sealed class CmdJail : Command2
    {
        ...
        void DoJail(Player p, string target, string[] args)
        {
            if (args.Length < 2) { Help(p); return; }
            TimeSpan duration = TimeSpan.Zero;
            if (!CommandParser.GetTimespan(p, args[1], ref duration, "jailed for", "m")) return;
        ...
        void DoUnjail(Player p, string target, string[] args)
        {
